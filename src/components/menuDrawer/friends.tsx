...
import right_arrow from '../../../public/icons/arrow.png';
...
export function MDFriends({ props, pages, efr, handleReferrals }:
  {
    ...
    efr: { earnedFromReferrals: number, setEarnedFromReferrals: React.Dispatch<any> }
    handleReferrals: (page?: number, take?: number) => Promise<TFriendsRequest | undefined>
  }) {

  const [friends, setFrends] = useState<{ balance: number; userName: string; }[]>([]);

  useEffect(() => {
    if (props?.friends) {
      // alert(JSON.stringify(props.friends))
      setFrends(props.friends);
    }
  }, [props?.friends]);
  ...
  const Pagination = async (where: string) => {
    if (where === 'next') {
      const tmp = await handleReferrals(page + 1, 10);
  ...
  return (
    <div className="w-full  flex flex-col mt-4 mb-4 overflow-auto h-[60%] px-2">
      ...
      <div className="w-full flex flex-col gap-2 items-center justify-center mt-2 mx-auto">
        <p className="text-[14px] pl-2 font-medium text-foreground">
          In addition to the bonus for inviting a friend, you also receive{' '}
          <span className="text-gold">
            {' '}
            0.5% of the balance of all your friends!
          </span>
        </p>
        <Button
          variant={'default'}
      ...
      <div className="w-[98%] flex flex-col items-center justify-center mt-4 mx-auto">
        <div className="w-[98%] inline-flex items-center justify-between">
          <p className="text-base pl-2 text-title-stroke font-medium text-foreground">
          ...
          {(props?.friends && props.friends.length > 0) ? (
            <div className="w-full flex flex-col overflow-x-hidden overflow-y-auto max-h-32">
              ...
              {friends.map((friend, idx) => (
                <div key={idx} className="w-full grid grid-cols-2">
                  <div className="max-w-full text-center text-normal-stroke">
