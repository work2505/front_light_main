...
export default function Bossdefeat(props: {
  ...
  const { setStateUser, user } = useData()!;

  const handleClaimReward = () => {
    setIsClaimed(true);
    let newObj: Tables<'users'>;
    if (user) {
      newObj = {
        ...user,
        balance: user?.balance + props.loot,
      };
      props.handleReward(false);
      setStateUser(newObj);
    }
  };
    ...
    <Drawer open={props.isOpen}>
      <DrawerContent>
        <div className="w-full inline-flex items-center justify-end relative pb-2">
          <div
            className={`w-full inline-flex items-center justify-center absolute  "top-[55%]"  left-1/2 -translate-x-1/2 -mt-9`}
          >
            <Image
          ...
          <h1
            className={`w-[60%] text-2xl font-medium text-center absolute left-1/2 -translate-x-1/2 top-[90%] -translate-y-1/2`}
          >
        ...
        <div className="w-full min-h-full max-h-full items-center flex flex-col mt-8 ml-2 overflow-auto ">
          <div className="flex pr-3  text-[14px]">
            <div className="bg-backdrop w-[128px] h-[128px] flex items-center flex-shrink-0 mr-[13px] rounded-[20px]">
              <img src="/icons/tresurechest.png" alt="tresurechest" />
            </div>
            <div>
              <span className="">
                If you defeat the monster every day, the loot will increase by{' '}
                <HoneyDisplay iconSize={16} amount={25000} /> each day..
              </span>
              <p className="mt-2">
                Come tomorrow and get{' '}
                <HoneyDisplay iconSize={16} amount={props.loot + 25000} /> . If
                you skip - progress will be lost
              </p>
            </div>
          </div>
          ...
          >
            {claim ? (
              <p className="opacity-60"> Come back in CD </p>
            ) : (
              <div className="flex items-center">
                ...
                <img
                  alt="honey"
                  src="/icons/honey.png"
                  className="ml-[2px] w-[20px] h-[20px] object-contain"
                />
              </div>
            )}
          </Button>
